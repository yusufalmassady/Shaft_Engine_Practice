...
public class DashBoard {
	...
	private By FastAccessWidgetremoteserviceselector=By.xpath(excelLocatorData.getCellData("FastAccessWidgetremoteserviceselector", "value"));
	...
	{
		driver.element()
		.scrollToElement(FastAccessWidgetremoteserviceselector)
		.select(FastAccessWidgetremoteserviceselector,ServiceName);
	}
	public By returnButtonPath(int buttonNum)
	{
		return By.id(excelLocatorData.getCellData("ButtonID", "value")+ButtonData.getCellData(buttonNum, 0).toString());
	}
	public void gotoUserScreen()
	{
		driver.element()
		.click(SiteManagement)
		.click(Users);
	}
	public void doFastActionsAssertions(int iterator)
	{
		By buttonElemnt=returnButtonPath(iterator);
		driver.element().scrollToElement(buttonElemnt)
		.assertThat(buttonElemnt).text()
